...
| Map-first PR review and deterministic local map | verified | `src/review_map`, `src/ui/review_map.rs`, `AppScreen` in `src/app.rs` | `tests/review_map_state.rs`, `tests/review_map_client.rs`, `tests/ui_review_map.rs`, `tests/ui_input.rs`, `tests/pty_pr.rs`, `tests/pty_ui.rs::local_diff_toggles_to_an_exact_review_map_and_back` |
...
| Loopback Review Map server/token/AI configuration | verified | typed fields and validation in `src/config`, bounded client in `src/review_map/client.rs` | `tests/config_resolution.rs::review_map_configuration_is_local_optional_and_validated`, `tests/review_map_client.rs` |
